...
public interface FriendRepository extends JpaRepository<FriendEntity, FriendId> { // <>안에는 Entity, Primarykey를 넣어준다.
    /** 친구 목록 */
    @Query(value = "select m from FriendEntity f " +
            "join MemberEntity m on f.friendId = m.memId " +
            "where f.memId=:loginId " +
            "order by m.nickname")
    ...
    @Query(value = "select m from FriendEntity f " +
            "join MemberEntity m on f.friendId = m.memId " +
            "where f.memId=:loginId " +
            "and m.nickname like %:friendName% " +
            "order by m.nickname")
    ...
    @Query(value = "select g.groupNumber, g.groupName from GroupPersonEntity p1 " +
            "join GroupPersonEntity p2 on p1.groupNumber = p2.groupNumber " +
            "join GroupEntity g on p1.groupNumber = g.groupNumber " +
            "where p1.memId=:memId and p2.memId=:friendId " +
            "and (p1.personStatus=1 or p1.personStatus=2) " +
